...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.custom_attn_inference_time = 0
        self.python_inference_time = 0
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            attF = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf'))
            attS = F.softmax(attF, dim=-1)
            attD = self.attn_dropout(attS)
            ...
            #c++ flash attention
            start_time = time.time()  # Store start time
            if self.testname == "part0":
                att2 = y_comp
            elif self.testname == "part1":
                # part1Test(N, d, B, H)
                temp = torch.zeros((N, N))
                att2 = ms.myNaiveAttention(q, k, v, temp, B, H, N, d)
            elif self.testname == "part2":
                # part2Test(N, d, B, H)
                temp = torch.zeros((N, N))
                att2 = ms.myUnfusedAttentionBlocked(q, k, v, temp, B, H, N, d)
            elif self.testname == "part3":
                # part3Test(N, d, B, H)
                temp = torch.zeros((NUM_THREADS, N))
                att2 = ms.myFusedAttention(q, k, v, temp, B, H, N, d)
            elif self.testname == "part4":
                # part4Test(N, d, B, H, block_size)
                bs = 128
                Qi = torch.zeros((bs, d))
                Kj = torch.zeros((bs, d))
                Vj = torch.zeros((bs, d))
                Sij = torch.zeros((bs, bs))
                Pij = torch.zeros((bs, bs))
                PV = torch.zeros((bs, d))
                Oi = torch.zeros((bs, d))
                L = torch.zeros((N))
                Lnew = torch.zeros((bs))
...
class GPT(nn.Module):
    ...
    @torch.no_grad()
    def generate(self, idx, max_new_tokens, decode, temperature=1.0, top_k=None):
        """
