...
class CustomAttention(nn.Module):
    ...
    def myUnfusedAttention(self):
        if self.isRef:
            with record_function("YOUR - NAIVE ATTENTION"):
    ...
    def myUnfusedAttentionBlocked(self):
        if self.isRef:
            with record_function("YOUR - BLOCKED MATMUL + UNFUSED SOFTMAX"):
                temp = torch.zeros((self.N, self.N))
    ...
    def myFusedAttention(self):
        if self.isRef:
            with record_function("YOUR - FUSED ATTENTION"):
    ...
    def myFlashAttention(self):
        ...
        if self.isRef:
            with record_function("YOUR - FLASH ATTENTION"):
...
def part1Test(N, d, B, H):
    ...
    Q,K,V = createQKVSimple(N,d,B,H)
    attentionModuleYour = CustomAttention(Q,K,V, B, H, N, d)
    attentionModuleReference = CustomAttention(Q,K,V, B, H, N, d, True)
    params = (N, d, B, H)
    print("-----RUNNING REFERENCE IMPLEMENTATION-----\n")
    testTemplate(attentionModuleYour.myUnfusedAttention, params, "REFERENCE - NAIVE ATTENTION")
    time.sleep(3)
    print("-----RUNNING YOUR IMPLEMENTATION-----\n")
    testTemplate(attentionModuleReference.myUnfusedAttention, params, "YOUR - NAIVE ATTENTION")
...
def part2Test(N, d, B, H):
    ...
    params = (N, d, B, H)
    print("-----RUNNING REFERENCE IMPLEMENTATION-----\n")
    testTemplate(attentionModuleYour.myUnfusedAttentionBlocked, params, "REFERENCE - BLOCKED MATMUL + UNFUSED SOFTMAX")
    time.sleep(3)
    print("-----RUNNING YOUR IMPLEMENTATION-----\n")
    testTemplate(attentionModuleReference.myUnfusedAttentionBlocked, params, "YOUR - BLOCKED MATMUL + UNFUSED SOFTMAX")
...
def part3Test(N, d, B, H):
    ...
    params = (N, d, B, H)
    print("-----RUNNING REFERENCE IMPLEMENTATION-----\n")
    testTemplate(attentionModuleYour.myFusedAttention, params, "REFERENCE - FUSED ATTENTION")
    time.sleep(3)
    print("-----RUNNING YOUR IMPLEMENTATION-----\n")
    testTemplate(attentionModuleReference.myFusedAttention, params, "YOUR - FUSED ATTENTION")

def part4Test(N, d, B, H, bc, br):
    print("Running Part 4 Test: Flash Attention\n")
    Q,K,V = createQKVSimple(N,d,B,H)
    attentionModuleYour = CustomAttention(Q,K,V, B, H, N, d, False, bc, br)
    attentionModuleReference = CustomAttention(Q,K,V, B, H, N, d, True, bc, br)
    params = (N, d, B, H)
    print("-----RUNNING REFERENCE IMPLEMENTATION-----\n")
    testTemplate(attentionModuleYour.myFlashAttention, params, "REFERENCE - FLASH ATTENTION")
    time.sleep(3)
    print("-----RUNNING YOUR IMPLEMENTATION-----\n")
    testTemplate(attentionModuleReference.myFlashAttention, params, "YOUR - FLASH ATTENTION")
...
def main():
    ...
    parser.add_argument("-m", "--model", default="shakes128", help="name of model to use: shakes128, shakes1024, shakes2048")
